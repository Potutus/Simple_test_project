...
const getAlbumMediaQuery = (
	...
	const query = `
        ...
        FilteredMedia AS (
            SELECT 
                Media.id,
                MAX(AlbumMedia."createdAt") AS added_at
            FROM media AS Media
            ...
            LEFT JOIN albums_media AS AlbumMedia ON AlbumMedia."mediaID" = Media.id
 
            CROSS JOIN params
            WHERE 
                AlbumMedia."albumID" = :albumID
                AND (
